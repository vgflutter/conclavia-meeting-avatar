...
The [Conclavia/Umatt deployment plan](https://github.com/vgflutter/conclavia-onboarding-avatar/blob/main/docs/production.md) covers domains, shared-kit releases, Mongo connectivity, backups and pilot acceptance. A custom domain does **not** inherit the development tunnel's management restrictions: authenticate management pages and APIs at the gateway before publishing Meeting. The workspace remains single-tenant. The subsequent deployment task activated release `20260926165443400-c14e5e` from clean application and kit commits on 26 September at https://meeting.conclavia.me and https://onboarding.conclavia.me with gateway authentication and the existing Mongo reached through a persistent SSH tunnel. AI and real bots remain disabled for this initial smoke test.
...
La release del 26 settembre include il kit `353bc3f`: sguardo, sorriso, guance e labbra vengono ricostruiti insieme nei saluti preregistrati, conservando l’espressione della performance completa. L’ascolto comprende sorrisi e cenni; il renderer operativo beta applica una chiusura esplicita per M/B/P anche quando il PCM ha energia nulla. Voci e durata dei saluti sono invariate. Rimangono alcune forme arrotondate attenuate e transizioni rapide; il renderer live resta distinto dalla qualità dei video preparati offline. Superati cinque test browser dell’host, 25 regressioni di playback e la build Linux coordinata con Onboarding. [Procedura e limiti visivi](https://github.com/vgflutter/conclavia-avatar-kit/blob/main/docs/host/coherent-performance.md).
