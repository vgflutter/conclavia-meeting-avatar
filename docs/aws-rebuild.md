...
4. Install the licensed RuntimeMetaHumanLipSync 1.0 package at
   `C:\ConclaviaStudio\Plugins\RuntimeMetaHumanLipSync`. Obtain it from its
...
7. From a clean, pushed Git commit run `npm run studio:source:deploy`. The script
   ...
   source audit.
8. Run the builders listed in `unreal/renderer-manifest.json` with
   `UnrealEditor-Cmd.exe` when generated content is absent, then validate with
   `Verify-SingleHeroReadiness.cjs`, the speech audit and a decoded browser run.
