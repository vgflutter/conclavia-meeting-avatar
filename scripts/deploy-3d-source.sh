...
const commands = [
  ...
  'Stop-ScheduledTask -TaskName "ConclaviaMeetingAvatarSupervisor" -ErrorAction SilentlyContinue',
  ...
  'Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -like "*C:\\ConclaviaStudio\\*" } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }',
  'New-Item -ItemType Directory -Path $root -Force | Out-Null',
  'if (-not (Test-Path (Join-Path $root "Content"))) { Copy-Item (Join-Path $legacyRoot "Content") $root -Recurse -Force }',
  'if (-not (Test-Path (Join-Path $root "Plugins"))) { Copy-Item (Join-Path $legacyRoot "Plugins") $root -Recurse -Force }',
  'New-Item -ItemType Directory -Path (Join-Path $root "Saved") -Force | Out-Null',
  'if (-not (Test-Path (Join-Path $root "Saved\\supervisor.token"))) { [Guid]::NewGuid().ToString("N") | Set-Content (Join-Path $root "Saved\\supervisor.token") -NoNewline -Encoding ASCII }',
  ...
  'Register-ScheduledTask -TaskName "ConclaviaMeetingAvatarSupervisor" -Action $taskAction -Principal $taskPrincipal -Settings $taskSettings -Trigger $taskTrigger -Force | Out-Null',
  'Disable-ScheduledTask -TaskName "ConclaviaStudioSupervisor" -ErrorAction SilentlyContinue | Out-Null',
  'Start-ScheduledTask -TaskName "ConclaviaMeetingAvatarSupervisor"',
