...
export function MeetingAgendaManager({ meetingId, initialAgenda }: { meetingId: string; initialAgenda: MeetingAgendaItem[] }) {
  ...
  const [localAgenda, setLocalAgenda] = useState<{ source: MeetingAgendaItem[]; value: MeetingAgendaItem[] }>();
  // A new server snapshot supersedes the local acknowledgement, including changes
  // made by a spoken command or another tab.
  const agenda = localAgenda?.source === initialAgenda ? localAgenda.value : initialAgenda;
  ...
  async function updateItem(itemId: string, status: AgendaItemStatus) {
    ...
    try {
      ...
      setLocalAgenda({ source: initialAgenda, value: payload.meeting.agenda });
      router.refresh();
    } catch {
      setError(
        isItalian
  ...
  return (
    <section className="card p-5 sm:p-7">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold">{isItalian ? "Scaletta" : "Agenda"}</h2>
        </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <p className={`break-words text-sm font-semibold ${item.status === "covered" ? "line-through decoration-emerald-400" : ""}`}>{item.title}</p>
                ...
                <div className="flex shrink-0 gap-2">
                  <button type="button" disabled={Boolean(pendingId)} onClick={() => updateItem(item.id, item.status === "covered" ? "pending" : "covered")} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold hover:bg-slate-50 disabled:opacity-50">{item.status === "covered" ? (isItalian ? "Riapri" : "Reopen") : (isItalian ? "Coperto" : "Covered")}</button>
                  {!item.mandatory && item.status !== "covered" && (
                    <button type="button" disabled={Boolean(pendingId)} onClick={() => updateItem(item.id, item.status === "skipped" ? "pending" : "skipped")} className="rounded-lg px-3 py-2 text-xs font-semibold text-slate-500 hover:bg-slate-100 disabled:opacity-50">{item.status === "skipped" ? (isItalian ? "Ripristina" : "Restore") : (isItalian ? "Salta" : "Skip")}</button>
                  )}
                </div>
              </div>
            </li>
