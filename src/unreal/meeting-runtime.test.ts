...
await test("keeps legacy podcast body assets out of the meeting runtime", async () => {
  ...
  assert.match(startScript, /L_MeetingAvatar_v22/);
  assert.match(engineConfig, /^GameDefaultMap=\/Game\/Conclavia\/Meeting\/L_MeetingAvatar_v22$/mu);
  assert.match(engineConfig, /^EditorStartupMap=\/Game\/Conclavia\/Meeting\/L_MeetingAvatar_v22$/mu);
  assert.match(stageBuilder, /^STAGE_REVISION = "v22"$/mu);
  ...
  assert.match(stageBuilder, /^MEETING_AVATAR_SCREEN_OFFSET_Y_CM = 7\.0$/mu);
