...
await test("keeps legacy podcast body assets out of the meeting runtime", async () => {
  ...
  assert.match(startScript, /L_MeetingAvatar_v23/);
  assert.match(engineConfig, /^GameDefaultMap=\/Game\/Conclavia\/Meeting\/L_MeetingAvatar_v23$/mu);
  assert.match(engineConfig, /^EditorStartupMap=\/Game\/Conclavia\/Meeting\/L_MeetingAvatar_v23$/mu);
  assert.match(stageBuilder, /^STAGE_REVISION = "v23"$/mu);
  ...
  assert.match(stageBuilder, /^MEETING_AVATAR_SCREEN_OFFSET_Y_CM = 9\.0$/mu);
  ...
  assert.match(startScript, /\$screenPercentage = if \(\$QualityPreset -eq "super"\) \{ 110 \}/);
