...
await test("keeps legacy podcast body assets out of the meeting runtime", async () => {
  ...
  assert.match(moduleSource, /TEXT\("amused"\),\s*0\.82f/su);
  ...
  assert.match(startScript, /L_MeetingAvatar_v12/);
  assert.match(engineConfig, /^GameDefaultMap=\/Game\/Conclavia\/Meeting\/L_MeetingAvatar_v12$/mu);
  assert.match(engineConfig, /^EditorStartupMap=\/Game\/Conclavia\/Meeting\/L_MeetingAvatar_v12$/mu);
  assert.match(stageBuilder, /^STAGE_REVISION = "v12"$/mu);
