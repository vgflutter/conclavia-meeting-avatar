...
test("animated portrait: four poses, expressions, no saves or voice calls", async ({ page, request }, info) => {
  ...
  for (const appearance of ["business_clay", "business_clay_female"]) {
    ...
    expect(imageUrl).toMatch(/^\/_next\/static\/media\/portraits-2-5d-v1/);
...
test("portrait mouth follows synthetic PCM and resets on stop", async ({ page }, info) => {
  ...
  await expect(canvas).toHaveAttribute("data-mouth-open", "1");
...
test("portrait image failure shows an explicit fallback, without changing the selection", async ({ page }) => {
  await page.route(/\/_next\/static\/media\/portraits-2-5d-v1[^/]*\.png/, route => route.abort());
  await page.goto("/avatar/test");
  await page.getByLabel("Avatar style", { exact: true }).selectOption("portrait_2_5d");
...
test("portrait respects reduced motion and fits both appearances and poses on small screens", async ({ page }, info) => {
  ...
  await expect.poll(() => portrait.getByTestId("portrait-canvas").getAttribute("data-head-tilt")).not.toBe("0.00000");
});
    ...
    for (const raised of [false, true]) {
      ...
      for (const tall of [true, false]) {
        ...
        const pixels = await portrait.locator("canvas").evaluate((c: HTMLCanvasElement, tall) => {
          ...
          return [.03, .1, .9, .97].map(n => [...ctx.getImageData(Math.floor(c.width * (tall ? .5 : n)), Math.floor(c.height * (tall ? n : .5)), 1, 1).data]);
        }, tall);
        expect(pixels).toEqual(Array(4).fill([251, 245, 234, 255]));
...
test("portrait blinks, moves and safely falls back on GPU context loss", async ({ page }, info) => {
  await page.goto("/avatar/test");
  await page.getByLabel("Avatar style", { exact: true }).selectOption("portrait_2_5d");
  ...
  for (const appearance of ["business_clay", "business_clay_female"]) {
    ...
    await expect.poll(() => canvas.getAttribute("data-head-tilt")).not.toBe("0.00000");
  }
  ...
  test(`portrait audio-clock sync and vowel textures: ${appearance}`, async ({ page }, info) => {
    ...
    for (const shape of ["a", "e", "o", "u"]) {
      ...
      const cheekError = await canvas.locator("canvas").evaluate(async (c: HTMLCanvasElement, female) => {
        ...
        const raised = c.parentElement!.dataset.handRaised === "true";
        const mouth = female ? raised ? [365, 256] : [325, 255] : raised ? [363, 273] : [313, 269];
        ...
        refCtx.drawImage(source, raised ? 627 : 0, female ? 627 : 0, 627, 627, dx, dy, side, side);
        let maxError = 0;
