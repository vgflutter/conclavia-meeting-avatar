...
$staging = 'C:\ConclaviaStudio\Saved\v26\ConclaviaLipSyncBridgeModule.cpp'
...
if (-not (Select-String -Path $staging -SimpleMatch 'v26-stable-animation-clock' -Quiet)) {
    throw 'The staged bridge is not the expected v26 revision.'
}

if (-not (Test-Path ($bridge + '.v25-root-teleport'))) {
    Copy-Item $bridge ($bridge + '.v25-root-teleport')
}
...
Write-Output 'V26_STABLE_ANIMATION_BUILD_OK'
