...
$staging = 'C:\ConclaviaStudio\Saved\v27\ConclaviaLipSyncBridgeModule.cpp'
...
if (-not (Select-String -Path $staging -SimpleMatch 'v27-native-body-additive' -Quiet)) {
    throw 'The staged bridge is not the expected v27 revision.'
}

if (-not (Test-Path ($bridge + '.v26-stable-animation-clock'))) {
    Copy-Item $bridge ($bridge + '.v26-stable-animation-clock')
}
...
Write-Output 'V27_NATIVE_BODY_ADDITIVE_BUILD_OK'
