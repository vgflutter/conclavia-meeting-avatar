...
$staging = 'C:\ConclaviaStudio\Saved\v31\ConclaviaLipSyncBridgeModule.cpp'
...
if (-not (Select-String -Path $staging -SimpleMatch 'v31-commercial-random-twelve-mood-test' -Quiet)) {
    throw 'The staged bridge is not the expected v31 random twelve-mood revision.'
}

if (-not (Test-Path ($bridge + '.v30-semantic-performance'))) {
    Copy-Item $bridge ($bridge + '.v30-semantic-performance')
}
...
Write-Output 'V31_RANDOM_TWELVE_MOODS_BUILD_OK'
