...
$staging = 'C:\ConclaviaStudio\Saved\v32\ConclaviaLipSyncBridgeModule.cpp'
...
if (-not (Select-String -Path $staging -SimpleMatch 'v32-commercial-deterministic-six-mood-test' -Quiet)) {
    throw 'The staged bridge is not the expected v32 deterministic six-mood revision.'
}

if (-not (Test-Path ($bridge + '.v31-random-twelve-moods'))) {
    Copy-Item $bridge ($bridge + '.v31-random-twelve-moods')
}
...
Write-Output 'V32_DETERMINISTIC_SIX_MOODS_BUILD_OK'
