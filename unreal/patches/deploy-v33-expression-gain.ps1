...
$stagingRoot = 'C:\ConclaviaStudio\Saved\v33'
$stagedBridge = Join-Path $stagingRoot 'ConclaviaLipSyncBridgeModule.cpp'
$stagedPatch = Join-Path $stagingRoot 'apply-expression-gain-v33.ps1'
$bridgeBackup = "$bridge.v32-before-expression-gain"
...
if (-not (Select-String -Path $stagedBridge -SimpleMatch 'v33-commercial-expression-gain' -Quiet)) {
    throw 'The staged bridge is not the expected v33 expression-gain revision.'
}
if (-not (Select-String -Path $stagedPatch -SimpleMatch 'Conclavia expression gain v33' -Quiet)) {
    throw 'The staged commercial-solver patch is not the expected v33 revision.'
}
...
try {
    ...
    & $build `
        ...
        -NoHotReload
    if ($LASTEXITCODE -ne 0) {
        throw "Unreal v33 build failed with exit code $LASTEXITCODE"
    }
}
catch {
    if (Test-Path "$pluginSource.v32-before-expression-gain") {
        Copy-Item "$pluginSource.v32-before-expression-gain" $pluginSource -Force
    }
...
Write-Output 'V33_EXPRESSION_GAIN_BUILD_OK'
